...
class MainController extends Controller {
    ...
    private function validation($request) {
        ...
        if (iconv_strlen($request['first_name']) < 2 || iconv_strlen($request['first_name']) > 150 || preg_match('/[^\w\d\-]+/u', $request['first_name'])) {
            $errors[] = [
                'type' => 'danger',
        ...
        if (iconv_strlen($request['last_name']) < 2 || iconv_strlen($request['last_name']) > 150 || preg_match('/[^\w\d\-]+/u', $request['last_name'])) {
            $errors[] = [
                'type' => 'danger',
